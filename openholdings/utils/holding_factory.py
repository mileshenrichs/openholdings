...
def is_bond(field_bag):
    return field_bag.quantity_held is not None
...
def create_bond_holding(field_bag):
    ...
    bond_fields = ['coupon_rate', 'rating', 'effective_date', 'maturity_date', 'next_call_date', 'quantity_held']
    copy_over_fields(field_bag, bond, COMMON_HOLDING_FIELDS + bond_fields)
    return bond

def create_future_holding(field_bag):
    return Future(field_bag.name)
